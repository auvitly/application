...
Если необходимо выполнить обработку сигнала внутри сервиса особым образом (например, отправка метрик), то
для этого служит метод `Panic()`, возвращающий канал для чтения, который может быть использован для 
создания обработчика этого события.